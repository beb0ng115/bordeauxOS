...
static void ReadAtVirtual(OpenFile *executable, int virtualaddr,
    ...
    int i;
    char buffer_tmp[numBytes];

    /* Sauvegarde des anciennes valeurs */
    TranslationEntry *pageTableSaved = machine->pageTable;
    unsigned pageTableSizeSaved = machine->pageTableSize;

    // On modifie la table des pages de la machine
    // Avec celle du processus
    machine->pageTable = pageTable;
    machine->pageTableSize = numPages;

    // On remplit le buffer temporaire
    executable->ReadAt(buffer_tmp, numBytes, position);

    // On écrit le buffer en mémoire
    for (i = 0; i < numBytes; ++i)
        // Comme on écrit des char, la taille est sizeof(char)
        machine->WriteMem(virtualaddr + i, sizeof (char), (int) buffer_tmp[i]);

    /* On remet l'état de la machine
       comme celle avant l'appel
     */
    machine->pageTable = pageTableSaved;
    machine->pageTableSize = pageTableSizeSaved;
